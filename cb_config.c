...
void cbSenhaAlterar(GtkButton *button, gpointer user_data)
{
  int i;
  struct strDB *sDB;
  char senha[100], sql[100], *SenhaAtual;
  ...
  if(i>0 && (sDB = MSSQL_Connect()) != NULL) {
    ...
    DB_GetNextRow(sDB, 0);
    SenhaAtual = MSSQL_GetData(0, 0);
    if(SenhaAtual && !strcmp(Crypto((char *)gtk_entry_get_text(GTK_ENTRY(gtk_builder_get_object(builder, "entSenhaAtual")))), SenhaAtual)) {
